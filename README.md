...
- 본인 사진을 기반으로 스타일 변형/캐릭터화한 이미지를 **여러 장** 준비해서
  `assets/persona_base/` 폴더 안에 커밋하세요 (예: `photo1.jpg`, `photo2.jpg`, ...).
  실행할 때마다 이 중 하나를 무작위로 골라 사용하므로, 각도/포즈가 다른 사진을
  여러 장 넣어둘수록 결과물이 더 다양해집니다.
- 반드시 실제 JPEG/PNG 형식으로 저장하세요 (확장자만 바꾸면 안 됩니다).
- 정면이 잘 보이고 배경이 단순한 사진일수록 이후 배경 합성 결과가 자연스럽습니다.
- 옷차림/표정은 코드가 장면에 맞게 자동으로 바꾸므로, 원본 사진의 옷차림이나
  표정은 크게 신경 쓰지 않으셔도 됩니다.
- 이 폴더 경로는 `PERSONA_BASE_IMAGE_DIR` 환경변수로 바꿀 수 있습니다.
